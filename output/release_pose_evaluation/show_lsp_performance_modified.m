...
ests_ori = load('/mnt/data/yfchen_data/Pose_Estimation/output/1/preds_mat/pred_lsp_pc_25000.mat'); ests_ori = ests_ori.pred;
% load ground truth, downloaded from http://www.comp.leeds.ac.uk/mat4saj/lsp.html
joints = load('lsp/joints.mat'); joints = joints.joints;
...
joints = joints(:,:,1001:2000);
% convert to the canonical joint order
joint_order_gt = [14,13,10,11,12,4,5,6,9,8,7,3,2,1];
ests = struct('joints', cell(size(joints,3),1));
gt = struct('joints', cell(size(joints,3),1));
for ii = 1:size(joints,3)
    gt(ii).joints = joints(1:2, joint_order_gt,ii)';
    ests(ii).joints = ests_ori(1:2, joint_order_gt,ii)';
end
