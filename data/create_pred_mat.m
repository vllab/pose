% use the output of c code to generate the pred .mat file
clear

num_model = '1';
iteration = '25000';

lsp_foldername = 'lsp_dataset/';
image_foldername = [lsp_foldername, 'images/'];
...
for i = 1:1000
    ...
    im = imread([image_foldername, im_name]);
    [h,w,~] = size(im);
...
%             keypoints_chosen_class(class,4) = coords_reshape(location+1,coords);
    ...
    % adjust the order
    pred(:,:,i) = keypoints_chosen_class([13 12 11 4 5 6 10 9 8 1 2 3 16 15], 2:3)';
end

% store the result
if(exist('iteration', 'var'))
    save([pred_mat_foldername, 'pred_lsp_pc_', iteration,'.mat'], 'pred');
else
    save([pred_mat_foldername, 'pred_lsp_pc.mat'], 'pred');
end
