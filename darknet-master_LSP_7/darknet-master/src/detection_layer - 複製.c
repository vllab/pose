...
detection_layer make_detection_layer(int batch, int inputs, int n, int side, int classes, int coords, int rescore, int miss, int numgc)
{
    detection_layer l = {0};
    l.type = DETECTION;
	...
	if (state.train){
		...
		*(l.cost) = 0;
		...
		for (b = 0; b < l.batch; ++b){
			int index = b*l.inputs;
			for (i = 0; i < locations; ++i) {
				int truth_index = (b*locations + i)*(1 + l.classes + l.coords*l.numgc);
				int is_obj = state.truth[truth_index];
				//cost_test = *l.cost;
				for (j = 0; j < l.n; ++j) {
					int p_index = index + locations*l.classes + i*l.n + j;
				...
				float best_rmse = 10;
				for (k = 0; k < l.numgc; k++){
					keypoint truth = float_to_keypoint(state.truth + truth_index + 1 + l.classes + k*l.coords);
					truth.x /= l.side;
					truth.y /= l.side;
					...
					for (j = 0; j < l.n; ++j){
						int box_index = index + locations*(l.classes + l.n) + (i*l.n + j) * l.coords;
						keypoint out = float_to_keypoint(l.output + box_index);
						out.x /= l.side;
						out.y /= l.side;

						float rmse = keypoint_rmse(out, truth);
						/*
						if (best_iou > 0 || iou > 0){
						if (iou > best_iou){
						best_iou = iou;
					...
					int keypoint_index = index + locations*(l.classes + l.n) + (i*l.n + best_index) * l.coords;
					int tkeypoint_index = truth_index + 1 + l.classes;

					/*
					keypoint out = float_to_keypoint(l.output + keypoint_index);
					out.x /= l.side;
					out.y /= l.side;
